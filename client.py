...
def generate_visualization(data: any, user_query: str, tool: str) -> tuple:
    ...
    system_prompt = """
                1.	Output only raw HTML, CSS, and JavaScript (no markdown, no explanations).
2.	Use Chart.js (CDN: https://cdn.jsdelivr.net/npm/chart.js) for all visualizations.
3.	Include at least 3 charts:
o	Bar chart
o	Line chart
o	Doughnut/Pie chart
4.	Each chart must follow the 6 subparts:
o	Structure → <canvas> element and chart instance.
o	Data → labels and datasets with values, colors.
o	Configuration → responsive, scales, plugins (title, legend, tooltip).
o	Styling → solid hex/rgba colors or Chart.js gradients created via ctx.createLinearGradient() (not CSS gradients).
o	Interactive → styled tooltips, hover effects, clickable legends.
o	Type → bar, line, doughnut/pie.

🎨 Chart Design Rules
•	Max chart height: 350–400px.
•	Use bright gradients and neon colors for clarity:
o	Bar chart → violet (#9b59b6), pink (#e91e63), blue (#2196f3).
o	Line chart → gradient from blue (#2196f3) to pink (#e91e63).
o	Doughnut chart → multiple distinct neon slices: violet, pink, blue.
•	Use borderWidth: 2 with opaque border colors.
•	Grid lines: rgba(255,255,255,0.1) for readability.
•	Tooltip background: dark with white text, rounded corners.
•	Charts must redraw responsively on window resize.
...
o	Average Price
o	Top Product
•	Show trend arrows (green ↑ for up, red ↓ for down).
•	Use gradient text for KPI values.
•	Smooth counter animation.
•	Add emoji/icons for visual appeal.

📋 Dynamic Table
•	Populate rows from a JSON dataset.
•	Include columns: Sale ID, Customer, Product, Qty, Unit Price, Total, Date, Email.
•	Add badges for email/status (✔ blue, ✖ orange).
•	Rows should have subtle hover effects.

✨ Styling Rules
•	Dark or gradient background for the body.
•	Glassmorphism cards: translucent, blurred, rounded corners, soft shadows.
•	Gradient text for headers and numbers.
•	Smooth hover effects on cards and rows.
•	Consistent accent palette (purple, pink, blue).

🌟 Advanced Polish
•	Show last updated timestamp in the header.
•	Add smooth animations (easeOutBounce, fade, grow).
•	Ensure all charts remain readable with clear axes, legends, and tooltips.

⚡ Final Deliverable:
A complete HTML document (raw HTML, CSS, and JS only) that runs directly in a browser, with colorful and professional charts like the uploaded image.
    """
